...
**Плечо ядра.** Две кнопки в интерфейсе задают, какое плечо в приоритете: верхнее
(p1, p3) или нижнее (p2, p4). Контроллер поднимает метрику IS-IS до 100 на всех
интерфейсах в сторону другого плеча, на всех восьми узлах ядра и в обе стороны.
Линки внутри самого плеча не трогаются, поэтому запасное плечо остаётся рабочим,
просто дорогим: при отказе приоритетного IGP сам уводит туда трафик, а когда оно
вернётся — возвращает обратно. Это и есть разница с явным путём: сеглист прибит
гвоздями и при отказе узла из пути уходит в чёрную дыру, метрика же только
предпочитает, а не запрещает.

Интерфейсы контроллер находит сам: берёт адрес линка из BGP-LS и ищет его в выводе
`ip -6 addr` на узле. Всё делается только в рантайме, в конфигах метрик нет, после
передеплоя лаба поднимается с равными путями и ECMP. Текущее состояние — `GET /api/leg`,
переключение — `POST /api/leg` с телом `{"leg": "upper" | "lower" | "none"}`.
...
```bash
bash te/traffic.sh start           # UDP 100 Мбит/с в 4 потока h1 -> h2, пока не остановишь
...
RATE=300M bash te/traffic.sh start # RATE суммарный, между потоками делится сам
STREAMS=1 bash te/traffic.sh start # один поток: пойдёт ровно одним путём
PROTO=tcp bash te/traffic.sh start
```
...
Один поток всегда идёт одним путём: ECMP хэширует поток целиком. Чтобы потоки
с разными портами расходились, в лабе включены два sysctl:

- `net.ipv4.fib_multipath_hash_policy=1` — хэш по L4 вместо L3. Работает на CE,
  где два пути до PE, и на PE, где два пути до дальнего CE.
- `net.ipv6.seg6_flowlabel=1` — узел, который инкапсулирует, считает flow label
  внешнего IPv6-заголовка из внутреннего пакета. Без этого в ядре все потоки для
  транзитных P одинаковы: портов во внешнем заголовке нет, за ним идёт SRH.
  По смыслу это entropy label из MPLS.

Так один поток занимает один путь через всю фабрику, а несколько расходятся
и на стыке с CE, и между PE, и внутри ядра. На схеме контроллера это видно
сразу: полосы появляются на нескольких линках и делят между собой полку.
...
Метрики: `lab_if_rx_bps`, `lab_if_tx_bps`, `lab_if_rx_pps`, `lab_if_tx_pps`, метки
`node`, `iface`, `peer`. Собираются только `eth1` и дальше, менеджмент `eth0` и `lo` — нет.

В базу идут не счётчики, а уже посчитанная скорость за интервал между снимками.
Счётчик плюс `rate()` на стороне базы — слишком хрупкая связка: любой скачок часов
ломает либо порядок точек, и тогда база видит сброс счётчика и рисует терабиты, либо
интервал, и тогда накопленные байты ложатся на несколько миллисекунд. Здесь интервал
считается по монотонным часам, которые назад не идут, а системные влияют только на то,
куда точка встанет по горизонтали. В запросах поэтому `avg_over_time`, а не `rate`.
